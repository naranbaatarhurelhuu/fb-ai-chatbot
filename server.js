...
// =====================
// ✅ ӨНГӨ БҮРИЙН ЖИЖИГ ЗУРГИЙН ЛИНК (SWATCH)
// ⚠️ Энд #1 link байхгүй бол хоосон "" хэвээр үлдээж болно.
...
// colors / colorImages-г бүгдэд нь нэгэн адил оноолоо
// =====================
const PRODUCTS = [
  {
    id: "MAXDROP",
...
// =====================
// Color carousel (scroll) + swatch image
// - FB generic template max 10 elements per message.
// - 10+ бол "▶ Дараагийн" дарж page-лэнэ.
// =====================
function sendColorCarouselPage(sender, productId, page = 0) {
...
function handlePostback(sender, payload) {
  ...
  if (payload.startsWith("PICK_")) {
    ...
    sendColorCarouselPage(sender, productId, 0);
    return;
  }

  // Color carousel paging
  if (payload.startsWith("COLOR_CAROUSEL_")) {
    const parts = payload.split("_"); // COLOR, CAROUSEL, productId, page
    const productId = parts[2];
    const page = parseInt(parts[3], 10) || 0;
    sendColorCarouselPage(sender, productId, page);
    return;
  }
...
function handleText(sender, textRaw) {
  ...
  // Хэн ч хэзээ ч бичсэн menu/start маягаар intro гарга
  const quick = lower.replace(/\s+/g, "");
  if (["сайнуу", "sainuu", "hi", "hello", "hey", "start", "эхлэх", "menu", "цэс", "?"].includes(quick)) {
    sessions.delete(sender);
    sendIntro(sender);
    return;
  }
  ...
  // Session байхгүй бол шууд intro
  if (!s) {
    sendIntro(sender);
    return;
  }
...
// FB Send helpers
// =====================
function fbSend(sender, message) {
  request(
    {
      uri: "https://graph.facebook.com/v19.0/me/messages",
      qs: { access_token: process.env.PAGE_TOKEN },
      method: "POST",
      json: { recipient: { id: sender }, message },
    },
    (err, resp, body) => {
...
// ✅ Buttons: type автоматаар postback болгож өгнө
function sendButtons(sender, text, buttons) {
  ...
  fbSend(sender, {
    attachment: {
      type: "template",
      payload: { template_type: "button", text, buttons: fbButtons },
    },
  });
}
